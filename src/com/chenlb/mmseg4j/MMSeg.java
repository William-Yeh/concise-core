...
public class MMSeg {
			...
			while(read && (data=readNext()) != -1) {
				...
				case Character.MODIFIER_LETTER:
					...
					case EN:
						...
						// 
						// 改成下面的 ReadCharByURL() 延伸原本的 ReadCharByAsciiOrDigit()
						ReadCharByURL rcad = new ReadCharByURL();
						readChars(bufSentence, rcad);
						if(rcad.hasDigit()) {
							wordType = Word.TYPE_LETTER_OR_DIGIT;
						}
						if (bufSentence.indexOf(".") != -1) {
							wordType = Word.TYPE_URL;
						}
						//only english
				...
				case Character.DECIMAL_DIGIT_NUMBER:
					...
					// 讀取後面的數字，包括decimal mark（千分位和小數點）
					readChars(bufSentence, new ReadCharByNumber());	// 讀取後面的數字，包括decimal mark（千分位和小數點）
				...
				default :
					// Added by Kuan-ming Su
					// 如果空格之外的，全都撿起來使用
					return new Word(Character.toChars(data), readedIdx, Word.TYPE_OTHER);					
	...
	private static abstract class ReadChar {
		 ...
		 * @throws IOException 
		 */
		abstract boolean isRead(int codePoint) throws IOException;
	...
	private static class ReadCharDigit extends ReadChar {
		...
		boolean isRead(int codePoint) throws IOException {
	 ...
	 * @author Kuan-ming Su.
	 */
	private class ReadCharByNumber extends ReadCharDigit {
		/** 新增判斷thousand separator和小數點 
		 * @throws IOException */
		boolean isRead(int codePoint) throws IOException {
			boolean isRead = super.isRead(codePoint);
			if (!isRead) {
				if ( codePoint == ',' || 	// thousand separator
					 codePoint == '.' )		// 小數點
				{	// 後面必須要接數字
					int data = -1;
					if ((data = readNext()) != -1) {
						int d = transform(data);
						isRead = isDigit(Character.getType(d));
						pushBack(data);
					}
				}
			}
	...
	/** 讀取網址 */
	private class ReadCharByURL extends ReadCharByAsciiOrDigit {
			...
			if (!isRead) {
				...
				if (codePoint == '/') {
					...
					if ((data = readNext()) != -1) {
						int d = transform(data);
						isRead = false;
						if (d == '/') {  // 第一個 slash (/) 後面要接另一個 slash (/)
							isRead = true;
						}
						
						// 檢查前面是不是 :// 
						// 後面要接數字或英文字
						else if (bufSentence.length() > 3 && 
								bufSentence.substring(bufSentence.length() - 2).equals(":/")) 
						{
							isRead = isDigit(Character.getType(data)) || isAsciiLetter(Character.getType(data));
						}
						pushBack(data);
					}
				}
				...
				if (codePoint == '.' || codePoint == '@') {
					...
					if ((data = readNext()) != -1) {
						int d = transform(data);
						isRead = d != Character.SPACE_SEPARATOR;
						pushBack(data);
					}
				}
			}
	...
	/**读取字母或数字*/
	private class ReadCharByAsciiOrDigit extends ReadCharDigit {
